...
def SSSS(topic, sub_keyword_list, year_from, year_to, citation_threshold, number_of_searches_per_key_word_per_year = 10, sleep_interval = 360):
    ...
    def detect_file_open(filename):  # Check if the file is open
        try:
            os.rename(filename, filename + '_temp')
            os.rename(filename + '_temp', filename)
        except OSError:
            print("\n**********************************************************\n{} is detected to be open. Please close the file \
                  before continuing...\n********************************************************** ".format(filename))
            
    # Generate a unique CSV file name with timestamp and random number
    current_time = dt.now().strftime('%Y%m%d_%H%M%S')
    random_number = int(random() * 10000)
    unique_filename = './results/topics/{}/summary_{}_{}.csv'.format(topic, current_time, random_number)
    ...
    if not os.path.exists(unique_filename):
        ...
        summary_df.to_csv(unique_filename, index = None, header = summary_df.columns)
    else:
        summary_df = pd.read_csv(unique_filename)
    ...
    for key_words in key_words_list:
        ...
        local_time()
        sleep_time = sleep_interval + random()*10
        ...
        for nth_paper in range(number_of_searches_per_key_word_per_year):
            ...
            print("summary_df的值========", summary_df.title.tolist(), "\n")

            # 判断文章标题是否不在 summary_df 数据中，且引用次数是否大于等于阈值 citation_threshold，如果满足条件，则继续处理。
            if (title_nth not in summary_df.title.tolist()) & (num_citations_nth >= citation_threshold):

                detect_file_open(unique_filename)
                ...
                summary_df.to_csv(unique_filename, index = False)
